...
#     (Synced with detect.py v2 — includes all Fixes #1-#5)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

class RedLightChecker:
    """
    Phát hiện xe vượt đèn đỏ — Đồng bộ đầy đủ logic từ detect.py v2.

    Bao gồm tất cả các Fix:
      [Fix #1] zone1_light_state: ghi trạng thái đèn lúc xe ở ROI1,
               dùng crossed_on_red thay vì is_red hiện tại
      [Fix #2] Hard transition detection: phát hiện đèn đổi đột ngột,
               reset smooth window ngay thay vì chờ majority vote
      [Fix #3] Grace period: không xóa state ngay khi xe mất track,
               chờ grace_period_frames trước khi dọn dẹp
      [Fix #4] Violators management: xóa sau cooldown để cho phép
               vi phạm lần 2 hợp lệ
      [Fix #5] Reset zone2_frame_count về âm sau vi phạm, đảm bảo
               đủ frame trước khi trigger lần tiếp
      [Fix gap] Cập nhật zone1_light_state khi xe ở vùng gap và
               đèn đã chuyển xanh → tránh false positive
    """
    ...
    def __init__(
        ...
        grace_period_frames: int = 50,      # [Fix #3] ~2s ở 25fps
        transition_confirm: int = 2,        # [Fix #2] số frame raw liên tiếp xác nhận đèn đổi
    ):
        ...
        self.transition_confirm = transition_confirm

        # State
        self.vehicles_in_zone1: set = set()
        self.zone1_light_state: dict = {}               # [Fix #1] track_id → trạng thái đèn lúc xe ở ROI1
        self.violators: set = set()
        self.zone2_frame_count: dict = defaultdict(int)
        self.last_violation_frame: dict = {}
        self.last_seen_frame: dict = {}                 # [Fix #3] track_id → frame gần nhất xe còn trong frame
        self.light_history: deque = deque(maxlen=smooth_window)
        self.current_light_status: str = "unknown"

        # [Fix #2] Hard transition detection
        ...
        self.light_roi_polygons: list = []              # Hỗ trợ nhiều vùng quét đèn
    ...
    def update_light_status(self, light_results, light_model, frame_height: int):
        ...
        Bao gồm [Fix #2] hard transition detection.
        ...
        for box in light_results.boxes:
            ...
            # Light ROI filtering — chỉ nhận đèn trong vùng ROI
            if self.light_roi_polygons:
                center_x = (x1 + x2) // 2
                in_any_roi = False
                for poly in self.light_roi_polygons:
        ...
        # ── [Fix #2] Hard transition detection ──────────────────
        # Nếu raw status đổi sang giá trị khác và liên tiếp >= transition_confirm
        # frame thì reset history ngay lập tức (tránh trễ khi đèn đổi màu)
        if raw_status != "unknown":
            ...
            if raw_status != current_smoothed:
                # Raw status đang khác với smoothed — đếm liên tiếp
                ...
                if self.raw_transition_count[raw_status] >= self.transition_confirm:
                    # Đèn thực sự đã đổi → reset window để cập nhật ngay
        ...
        # Trạng thái đèn chính thức = majority vote trong window
    ...
    def check_vehicles(
        ...
        """
        Kiểm tra xe vượt đèn đỏ — Logic đồng bộ detect.py v2.
        Bao gồm Fix #1, #3, #4, #5 và gap logic.
        """
        ...
        for box, track_id, cls_id, conf in zip(
            ...
            # ── [Fix #1] Cập nhật trạng thái đèn MỖI LẦN xe ở zone1 ──
            # Luôn ghi lại trạng thái đèn hiện tại (mới nhất khi xe ở vạch)
            if inside_roi1:
                self.zone1_light_state[track_id] = self.current_light_status
                self.vehicles_in_zone1.add(track_id)
                # Reset zone2 counter vì xe còn ở zone1 (chưa vượt qua)
                self.zone2_frame_count[track_id] = 0

            # ── [Fix gap] Xe ở vùng gap (không zone nào) ──
            if not inside_roi1 and not inside_roi2:
                if track_id in self.vehicles_in_zone1:
                    # Xe đã qua zone1, đang đi về zone2 nhưng chưa tới.
                    # Nếu đèn đã chuyển xanh trong lúc xe ở gap → cập nhật lại.
                    # Tránh false positive: xe đứng ở đó lúc đỏ, đèn chuyển xanh,
                    # xe mới chạy → gap không cập nhật → vào zone2 vẫn bị flag.
                    if self.current_light_status in ("green", "yellow", "off") \
                       and self.current_light_status != "unknown":
                        self.zone1_light_state[track_id] = self.current_light_status
                else:
                    # Xe chưa từng vào zone1 → xóa state nếu có
                    if self.zone2_frame_count[track_id] == 0 \
                       and track_id not in self.violators:
                        self.zone1_light_state.pop(track_id, None)

            # Cập nhật frame counter zone2
            if inside_roi2:
                self.zone2_frame_count[track_id] += 1
            elif not inside_roi1:
                # Xe rời zone2 mà không vào zone1 → reset counter
                self.zone2_frame_count[track_id] = 0

            # ── [Fix #4] Quản lý violators chính xác ──
            # Giữ trong violators cho đến khi hết cooldown
            # Sau cooldown → xóa khỏi violators để xe có thể bị ghi nhận vi phạm mới
            ...
            if track_id in self.violators and cooldown_ok:
                # Hết cooldown → "ân xá" để có thể vi phạm mới nếu vượt đèn đỏ lần nữa
                self.violators.discard(track_id)

            consecutive = self.zone2_frame_count[track_id]
            crossed_on_red = self.zone1_light_state.get(track_id) == "red"  # [Fix #1]

            # ── KIỂM TRA ĐIỀU KIỆN VI PHẠM ──
            violation_detected = (
                crossed_on_red                                    # 1. Đèn ĐỎ lúc xe đứng ở vạch (ROI1)
                and inside_roi2                                   # 2. Xe đang trong vùng vi phạm (ROI2)
                and track_id in self.vehicles_in_zone1            # 3. Xe đã đi từ Zone1 (đúng hướng)
                and consecutive >= self.frame_threshold           # 4. Đủ số frame liên tiếp
                and cooldown_ok                                   # 5. Hết cooldown
                and track_id not in self.violators                # 6. [Fix #4] Chưa đang trong cooldown
            )
            ...
            if violation_detected:
                ...
                # [Fix #5] Reset về âm để đảm bảo đủ frame_threshold trước khi trigger lại
        ...
        # ── [Fix #3] Dọn dẹp state với grace period ──
        self._cleanup_stale_tracks(current_ids, frame_number)

        return violations

    def _cleanup_stale_tracks(self, current_track_ids: set, frame_number: int):
        """
        [Fix #3] Dọn state xe vắng mặt quá grace period.
        Không xóa state ngay khi xe mất khỏi frame.
        Chỉ xóa sau khi xe vắng mặt quá grace_period_frames.
        Tránh mất vi phạm khi ByteTrack mất ID 1-2 frame.
        """
        all_tracked = set(self.vehicles_in_zone1) | set(self.zone1_light_state.keys())
        for tid in list(all_tracked):
            if tid in current_track_ids:
                continue  # Xe vẫn trong frame, bỏ qua

            frames_absent = frame_number - self.last_seen_frame.get(tid, frame_number)
            if frames_absent > self.grace_period_frames:
                # Xe vắng mặt quá lâu → xóa sạch state
                ...
                # Dọn dẹp last_violation_frame nếu quá cooldown
