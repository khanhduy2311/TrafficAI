"""
He thong phat hien vuot den do 
================================================
Su dung: python detect_traffic_sign.py --source video.mp4 --light-weights models/light.pt --vehicle-weights models/vehicle.pt
    ...
    while cap.isOpened():
        ...
        if vehicle_results.boxes.id is not None:
            ...
            for box, track_id, cls_id, det_conf in zip(boxes, track_ids, classes, confs):
                ...
                # Cap nhat lich su zone1
                if inside_roi1:
                    vehicles_in_zone1.add(track_id)

                # Reset trang thai zone1 khi xe da hoan toan roi khoi ca 2 vung
                ...
                violation_detected = (
                    is_red                                      # 1. Dang la den do
                    and inside_roi2                             # 2. Xe dang trong vung vi pham
                ...
                if violation_detected:
                    ...
                    print(f"[Frame {frame_count}] *** VI PHAM *** ID:{track_id} | {cls_name} | {timestamp_str}")
                    ...
                    cv2.putText(evidence_frame, f"DEN DO - {timestamp_str}", (30, 60),
        ...
        # Reset state cho xe da bien mat khoi frame qua nhieu frame lien tiep
        # (ByteTrack tu quan ly, nhung minh van can don dep set)
        gone_ids = set(vehicles_in_zone1) - current_track_ids
        for tid in gone_ids:
            # Khong xoa violators (de giu mau do neu xe xuat hien lai)
            # Nhung xoa zone1 state de tranh beo cung set
            pass  # ByteTrack tu reset track ID khi mat qua lau
